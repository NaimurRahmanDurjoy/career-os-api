...
public function upload(Request $request)
    ...
    public function storeBuiltResume(Request $request)
    {
        $request->validate([
        ...
        try {
            $resume = Resume::create([
                'user_id' => $user->id,
                'file_name' => 'ATS_CV_Builder.pdf', // Virtual filename, used by UI 
                'file_path' => 'cv_builder_virtual', 
                'version_name' => $request->version_name ?? 'Built CV',
                'status' => 'completed',
                'ats_score' => 95, 
                'parsed_content' => [
                    'status' => 'completed',
                    'structured_data' => $request->cv_data
                ],
                'ai_suggestions' => [
                    ['category' => 'General', 'suggestion' => 'This resume was generated by the ATS Builder and is natively optimized.']
                ]
            ]);

            return response()->json([
                'success' => true,
                'message' => 'Builder resume saved successfully.',
                'resume' => $resume
            ], 201);
        } catch (Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Failed to save built resume.',
                'error' => $e->getMessage()
            ], 500);
        }
    }
